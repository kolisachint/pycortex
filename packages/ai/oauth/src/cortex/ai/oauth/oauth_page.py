"""OAuth page HTML templates.

Mechanical port of hoocode's ``packages/ai/src/utils/oauth/oauth-page.ts``.
"""

from __future__ import annotations

LOGO_SVG = (
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 800" aria-hidden="true">'
    '<path fill="#fff" fill-rule="evenodd" '
    'd="M165.29 165.29 H517.36 V400 H400 V517.36 H282.65 V634.72 H165.29 Z '
    'M282.65 282.65 V400 H400 V282.65 Z"/>'
    '<path fill="#fff" d="M517.36 400 H634.72 V634.72 H517.36 Z"/>'
    "</svg>"
)


def _escape_html(value: str) -> str:
    """Escape HTML special characters."""
    return (
        value.replace("&", "&amp;")
        .replace("<", "&lt;")
        .replace(">", "&gt;")
        .replace('"', "&quot;")
        .replace("'", "&#39;")
    )


def _render_page(title: str, heading: str, message: str, details: str | None = None) -> str:
    """Render an OAuth page."""
    title_escaped = _escape_html(title)
    heading_escaped = _escape_html(heading)
    message_escaped = _escape_html(message)
    details_escaped = _escape_html(details) if details else None

    return f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>{title_escaped}</title>
  <style>
    :root {{
      --text: #fafafa;
      --text-dim: #a1a1aa;
      --page-bg: #09090b;
      --font-sans: ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto,
        "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji",
        "Segoe UI Symbol", "Noto Color Emoji";
      --font-mono: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono",
        "Courier New", monospace;
    }}
    * {{ box-sizing: border-box; }}
    html {{ color-scheme: dark; }}
    body {{
      margin: 0;
      min-height: 100vh;
      display: flex;
      align-items: center;
      justify-content: center;
      padding: 24px;
      background: var(--page-bg);
      color: var(--text);
      font-family: var(--font-sans);
      text-align: center;
    }}
    main {{
      width: 100%;
      max-width: 560px;
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: center;
    }}
    .logo {{
      width: 72px;
      height: 72px;
      display: block;
      margin-bottom: 24px;
    }}
    h1 {{
      margin: 0 0 10px;
      font-size: 28px;
      line-height: 1.15;
      font-weight: 650;
      color: var(--text);
    }}
    p {{
      margin: 0;
      line-height: 1.7;
      color: var(--text-dim);
      font-size: 15px;
    }}
    .details {{
      margin-top: 16px;
      font-family: var(--font-mono);
      font-size: 13px;
      color: var(--text-dim);
      white-space: pre-wrap;
      word-break: break-word;
    }}
  </style>
</head>
<body>
  <main>
    <div class="logo">{LOGO_SVG}</div>
    <h1>{heading_escaped}</h1>
    <p>{message_escaped}</p>
    {f'<div class="details">{details_escaped}</div>' if details_escaped else ""}
  </main>
</body>
</html>"""


def oauth_success_html(message: str) -> str:
    """Render a success page."""
    return _render_page(
        title="Authentication successful",
        heading="Authentication successful",
        message=message,
    )


def oauth_error_html(message: str, details: str | None = None) -> str:
    """Render an error page."""
    return _render_page(
        title="Authentication failed",
        heading="Authentication failed",
        message=message,
        details=details,
    )
